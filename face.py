...
IMAGE_FILE = "./dataset/cap_010_full.jpg"
...
if face_count > 0:
    image_copy = np.copy(image.numpy_view())  # Make a copy of the image
    annotated_image = visualize(image_copy, detection_result)  # Visualize the detection result
    rgb_annotated_image = cv2.cvtColor(annotated_image, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
    ...
    has_full_face = bbox.box_touches_edge(image_width=image_width, image_height=image_height)
...
hsv = cv2.cvtColor(image_copy, cv2.COLOR_BGR2HLS)
...
print("Has full face:", has_full_face)
